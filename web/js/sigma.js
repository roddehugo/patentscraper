  ...
  dataReady = function() {//This is called as soon as data is loaded
    ...
    a.iterNodes(
      function (b) { //This is where we populate the array used for the group select box
        ...
        // alert(b.x);
        mod = b.attr.attributes[config.features.groupSelectorAttribute];
        modName = "Group" + mod;
        a.clusters[modName] || (a.clusters[modName] = {
          nodes: [],
          name: config.classes[mod],
          color: b.color
        });
        a.clusters[modName].nodes.push(b.id);//SAH: push id not label
...
function setupGUI(config) {
  ...
  $("#maintitle").html("<h2>"+config.text.maintitle+"</h2>");
  $("#title").html("<h3>"+config.text.title+"</h3>");
  ...
  $GP.info_close2 = $GP.info.find(".select-close");
...
function configSigmaElements(config) {
  ...
  var a = [],
  b,x=1;
  for (b in sigInst.clusters) a.push('<div style="line-height:12px"><a href="#' + b + '"><div style="width:40px;height:12px;border:1px solid #fff;background:' + sigInst.clusters[b].color + ';display:inline-block"></div> ' + sigInst.clusters[b].name + ' (' + sigInst.clusters[b].nodes.length + ' members)</a></div>');
...
function Cluster(a) {
  ...
  this.select.html('Select ' + toTitleCase(config.features.groupSelectorAttribute));
  ...
  this.hide = function () {
    this.display = !1;
    this.list.hide();
    this.select.removeClass("select-close")
  };
  this.show = function () {
    this.display = !0;
    this.list.show();
    this.select.addClass("select-close")
  }
}
...
function nodeActive(a) {
  ...
  var createList=function(c) {
    ...
    for (g in c) {
      ...
      a != g && e.push({
        id: g,
        name: d.label,
        title: d.attr.attributes['title'],
        type: d.attr.attributes['type'],
    ...
    e.sort(function (a, b) {
      ...
      g = a.type.toLowerCase(),
      h = b.type.toLowerCase();
      return g == h ? c != d ? c < d ? -1 : c > d ? 1 : 0 : e < f ? -1 : e > f ? 1 : 0 : 0
    });
    d = "";
    for (g in e) {
      c = e[g];
      name = c.title || c.name || c.id;
      f.push('<li class="membership"><a href="#' + name + '" onmouseover="sigInst._core.plotter.drawHoverNode(sigInst._core.graph.nodesIndex[\'' + c.id + '\'])\" onclick=\"nodeActive(\'' + c.id + '\')" onmouseout="sigInst.refresh()">' + name + "</a></li>");
    }
    return f;
  }
  ...
  if (f.attributes) {
    ...
    for (var attr in f.attributes) {
      ...
      if (attr!=image_attribute) {
        h = '<span><strong>' + toTitleCase(attr) + ':</strong> ' + d + '</span>'
      }
      e.push(h)
    }
    ...
    label = b.label || b.id;
    if (image_attribute) {
      $GP.info_name.html("<div><img src=" + f.attributes[image_attribute] + " style=\"vertical-align:middle\" /> <span onmouseover=\"sigInst._core.plotter.drawHoverNode(sigInst._core.graph.nodesIndex['" + b.id + '\'])" onmouseout="sigInst.refresh()">' + label + "</span></div>");
    } else {
      $GP.info_name.html("<div><span onmouseover=\"sigInst._core.plotter.drawHoverNode(sigInst._core.graph.nodesIndex['" + b.id + '\'])" onmouseout="sigInst.refresh()">' + label + "</span></div>");
    }
  ...
  sigInst.active = a;
  window.location.hash = b.label || b.id;
}

function showCluster(a) {
  var b = sigInst.clusters[a];
  if (b && b.nodes && 0 < b.nodes.length) {
    showGroups(!1);
    sigInst.detail = !0;
    b.nodes.sort();
    ...
    for (var f = [], e = [], c = 0, g = b.nodes.length; c < g; c++) {
      var d = sigInst._core.graph.nodesIndex[b.nodes[c]];
      label = d.attr.attributes['title'] || d.label || d.id;
      !0 == d.hidden && (e.push(b.nodes[c]), d.hidden = !1, d.attr.lineWidth = !1, d.attr.color = d.color, f.push('<li class="membership"><a href="#'+label+'" onmouseover="sigInst._core.plotter.drawHoverNode(sigInst._core.graph.nodesIndex[\'' + d.id + "'])\" onclick=\"nodeActive('" + d.id + '\')" onmouseout="sigInst.refresh()">' + label + "</a></li>"))
    }
    sigInst.clusters[a].nodes = e;
    sigInst.draw(2, 2, 2, 2);
    $GP.info_name.html("<b>" + b.name + "</b>");
    $GP.info_data.hide();
